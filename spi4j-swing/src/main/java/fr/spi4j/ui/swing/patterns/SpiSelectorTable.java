...
/**
 * Composant de type table permettant de sélectionner certains éléments au moyen d'une colonne cases à cocher. <br/>
 * C'est donc une alternative à une table en sélection multiple ou à un composant de type mouvementeur.
 * @param <TypeValue>
 *           Type des valeurs de la liste
 * @author MINARM
 */
public class SpiSelectorTable<TypeValue> extends SpiTable_Abs<TypeValue> implements HasMultipleSelection_Itf<TypeValue>
{
   ...
   /**
    * TableModel gérant spécifiquement la colonne 0 pour les valeurs des sélections.
    * @param <TypeValue>
    *           Type des valeurs de la liste
    * @author MINARM
    */
   public static class SelectorTableModel<TypeValue> extends SpiTableModel<TypeValue>
   {
      ...
      /**
       * Méthode appelée lors de la sélection d'une case à cocher pour envoyer un évènement cellUpdated au modèle. <br/>
       * Cet appel sur le modèle provoque un tri du tableau (par ex. sur la colonne Sél.) et un réaffichage.<br/>
       * Mais si cela ne convient pas (par ex. si le réaffichage à chaque sélection ne convient pas), cette méthode peut être surchargée.
       * @param row
       *           int
       * @param column
       *           int
      ...
      {
         // sans cela, certaines cases ne sont pas réafficher correctement sur un drag de souris
   ...
   private static class MyTableColumnModel extends DefaultTableColumnModel
   {
      ...
      {
         // Cette méthode est redéfinie pour ne pas enlever la colonne de modelIndex == 0 (colonne de sélection).
   ...
   public SpiSelectorTable ()
   {
      ...
      // BOOLEAN_TABLE_CELL_EDITOR ne fonctionne pas bien en Java 1.4 (sur les lignes sélectionnées)
      // et en plus ce MOUSE_MOTION_HANDLER fonctionne encore mieux que ce dernier (pas besoin de cliquer les lignes une par une)
   ...
   /**
    * Création du modèle de données par défaut. <br/>
    * Le modèle par défaut est un SpiTableModel spécifique pour la gestion de la colonne de sélection.
    * @return TableModel
    */
   @Override
   ...
   /**
    * Retourne l'instance de SpiSelectorTable père du composant source de l'événement spécifié.
   ...
   public List<TypeValue> getSelectionList ()
   {
      ...
      // Math.min nécessaire si réentrant sur getSelectionList avant setSelectionList dans setList
   ...
   public void setSelectionList (final List<TypeValue> newSelectionList)
   {
      ...
      if (newSelectionList != null)
      {
         ...
         for (int i = 0; i < size; i++)
         {
            ...
            {
               // si l'objet à sélectionner n'a pas été trouvé et si c'est un DTO avec un id, on le recherche par son identifiant
   ...
   /**
    * Création du modèle de données par défaut. <br/>
    * Le modèle par défaut est particulier pour SpiSelectorTable.
