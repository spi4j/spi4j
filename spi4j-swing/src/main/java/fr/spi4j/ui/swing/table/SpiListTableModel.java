...
public abstract class SpiListTableModel<TypeValue> extends AbstractTableModel {
	...
	public TypeValue getObjectAt(final int rowIndex) {
		if (rowIndex < 0 || rowIndex >= getRowCount()) {
			...
			// qui s'est manifesté dans COSI pour la sélection des composants dans QuiOu
			// d'une opération d'infrastructure
			// (Mantis 910,
			// https://safran-forge-sid.sid.defense.gouv.fr/mantis/view.php?id=910)
	...
	/**
	 * Retourne un booléen suivant que la cellule est éditable (false par défaut).
	 * <p>
	 * Ici, l'implémentation est faite selon que la propriété cellEditor de la
	 * TableColumn correspondante est nulle ou non.
	 * 
	 * @return boolean
	 * @param row    int
	 * @param column int
