 ...
 * <p>
 * Les méthodes addItem, insertItemAt, removeItem, removeItemAt, removeAllItems
 * ne peuvent être utilisées car le model n'est pas 'mutable'. Il faut utiliser
 * getList et setList pour définir les données affichées.
 * <p>
 * Et il faut utiliser la méthode setRenderer avec une implémentation héritant
 * de SpiComboBoxRenderer_Abs ou de SpiDefaultListCellRenderer pour définir le
 * rendu. Les méthodes get/set pour selectedItem et selectedIndex ne peuvent
 * être utilisées car elles sont appelées en interne Swing et qu'elle ne gère
 * pas la valeur nulle.
 * <p>
 * Il faut utiliser getSelectedObject et setSelectedObject.
 * 
...
public class SpiComboBox<TypeValue> extends JComboBox implements HasSelection_Itf<TypeValue> {
		...
		public void itemStateChanged(final ItemEvent itemEvent) {
			if (itemEvent.getSource() instanceof JComboBox) {
				...
				if (selectedItem != null && SpiComboBoxModel.NULL_VALUE != selectedItem) {
					...
					if (text != null) {
						// s'il y a un renderer contenant un texte comme un label (cas extrêmement
						// probable)
						// alors le toolTip est le texte du renderer
						// (par défaut sur les SpiComboBox, il y a un renderer de type label qui utilise
						// le toString() sur l'élément)
						toolTipText = text;
					} else {
	...
	public SpiComboBox() {
		...
		// on fixe la taille préférée pour éviter que les largeurs des comboBox et des
		// écrans soient infiniment grandes selon les textes
		setPreferredSize(getSize());
		addItemListener(ITEM_HANDLER);
		setRenderer(DEFAULT_RENDERER);
		// ligne ci-dessous nécessaire sinon createDefaultKeySelectionManager() n'est
		// pas appelé et le manager est celui dans UI
	 ...
	 * <p>
	 * L'objet peut être null et il peut ne pas être dans la liste. Si il n'est pas
	 * dans la liste, il est ajouté temporairement à la fin de la liste et enlevé
	 * dès qu'un autre objet est sélectionné (par le programme, pas par
	 * l'utilisateur).
	 * <p>
	...
	public void setSelectedObject(final TypeValue newSelectedObject) {
		...
		if (index == -1 && newSelectedObject instanceof Dto_Itf) {
			// si l'objet à sélectionner n'a pas été trouvé et si c'est un DTO, on le
			// recherche par son identifiant
			// (car il peut arriver d'avoir deux instances du DTO avec le même identifiant,
			// y compris pour le "référentiel")
			final Dto_Itf<?> dto = DtoUtil.findInCollectionById((List<Dto_Itf<?>>) list,
					(Dto_Itf<?>) newSelectedObject);
			index = list.indexOf(dto);
		}
		if (index == -1 && newSelectedObject instanceof Entity_Itf) {
			// si l'objet à sélectionner n'a pas été trouvé et si c'est un Entity, on le
			// recherche par son identifiant
			// (car il peut arriver d'avoir deux instances du Entity avec le même
			// identifiant, y compris pour le "référentiel")
		...
		if (newSelectedObject == null || index >= 0) {
			...
			// il apparaîtra à la fin (voir SpiComboBoxModel.getSize et
			// SpiComboBoxModel.getElementAt)
	 ...
	 * 
	 * @param newNullValueDisplayed boolean
	 ...
	 * 
	 * @param renderer SpiComboBoxRenderer_Abs
	 */
	@SuppressWarnings("unchecked")
	...
	public Dimension getSize() {
		...
		// (workaround RFE 4618607)
		final Dimension dim = super.getSize();
		if (!layingOut) {
			// calcul de la taille si affichage de la popup déroulante (pour que la popup
			// soit plus grande si besoin selon les textes)
			Dimension preferredSize = null;
			if (ui != null) {
				// on préfère ui.getPreferredSize, plutôt que getPreferredSize qui a été définie
				// dans le constructeur
				// (getPreferredSize n'est plus une valeur dépendante du texte mais seulement
				// une valeur raisonnable pour la comboBox sans popup)
	...
	/**
	 * Vérifie si une valeur est nulle (en testant en plus l'objet
	 * <code>{@link SpiComboBoxModel}.NULL_VALUE</code>)
	 * 
	 * @param p_value la valeur saisie dans la SpiTable
	 * @return true si la valeur saisie dans la table est nulle (peut être la valeur
	 *         nulle par défaut d'une combobox)
	 */
