...
public class SpiStringArea extends JScrollPane implements HasString_Itf, HasMaxLength_Itf {
	...
	/**
	 * Retourne l'instance de SpiStringArea père du composant source de l'événement
	 * spécifié.
	 * 
	...
	public void setEditable(final boolean newEditable) {
		...
		// appel de updateBackground car sans cela la couleur de fond du JTextArea non
		// éditable reste blanche et non grisée
		updateBackground(myTextArea);
	}

	/**
	 * Méthode inspirée directement de
	 * {@link javax.swing.plaf.basic.BasicTextUI#updateBackground}, mais
	 * contrairement à celle-ci nous forçons son exécution si Nimbus (à base de
	 * Synth) et si c'est un JTextArea (BasicTextUI#updateBackground se désactive
	 * malheureusement dans ces 2 cas).
	 * <p>
	 * Updates the background of the text component based on whether the text
	...
	static void updateBackground(final JTextComponent c) {
		...
		if (background instanceof UIResource) {
			...
			// final In an final ideal situation, the following final check would not final
			// be necessary and final we would replace final the color any final time the
			// previous final color was a UIResouce.
			// However, it turns final out that there final is existing code final that uses
			// the final following inadvisable pattern final to turn a final text area into
			// final what appears to final be a multi-line label:
			//
			...
			//
			// JLabel's default background is a UIResource. As such, just checking for
			// UIResource would have us always changing the background away from what the
			// developer wanted.
			// Therefore, for JTextArea/JEditorPane, we'll additionally check that the color
			// we're about to replace matches one that was installed by us from the
			// UIDefaults.
