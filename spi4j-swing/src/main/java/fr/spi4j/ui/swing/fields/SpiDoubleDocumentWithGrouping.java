...
/**
 * Document swing alternatif à SpiDoubleDocument pour la saisie d'un nombre
 * décimal, et qui affiche et accepte en saisie des séparateurs de milliers.
 * <p>
 * Pour l'utiliser, appeler au démarrage de l'application :
 * System.setProperty("fr.spi4j.ui.swing.fields.SpiDoubleField.documentType",
 * "fr.spi4j.ui.swing.fields.SpiDoubleDocumentWithGrouping");
 * 
 * @author MINARM
 */
public class SpiDoubleDocumentWithGrouping extends SpiDoubleDocument {
	...
	public void insertString(final int offset, final String string, final AttributeSet attributeSet)
		...
		// même si l'utilisateur saisi un '.' ou un ' ', on les remplace par ',' et par
		// l'espace insécable
		...
		isOK = isOK && value >= getMinimumValue() && value <= getMaximumValue()
				&& (text.indexOf(DECIMAL_SEPARATOR) == -1
						|| (text.length() - text.indexOf(DECIMAL_SEPARATOR) - 2 < getFractionDigits()
								&& getFractionDigits() > 0))
