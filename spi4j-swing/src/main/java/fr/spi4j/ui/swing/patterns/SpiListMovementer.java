...
/**
 * Composant mouvementeur permettant de choisir certains éléments parmi une
 * liste d'éléments disponibles et des les déplacer dans une liste d'éléments
 * sélectionnés.
 * <p>
 ...
 * @author MINARM
 */
public class SpiListMovementer<TypeValue> extends JPanel
		implements ActionListener, HasMultipleSelection_Itf<TypeValue> {
	/**
	...
	private final class ListMovementerTransferHandler extends TransferHandler {
		...
		public boolean canImport(final TransferSupport support) {
			...
			return !getTargetTableFromSupport(support).getList()
					.containsAll(getTransferDataFromTransferable(support.getTransferable()))
	 ...
	 */
	public static final DataFlavor LIST_MOVEMENTER_FLAVOR = new DataFlavor(
			"application/x-" + SpiListMovementer.class.getSimpleName().toLowerCase(), "Liste à déplacer");
	...
	private static final Icon LEFTLEFT_ICON = new ImageIcon(
			SpiListMovementer.class.getResource("/icons/LeftLeft16.gif"));
	...
	/**
	 * Définit la valeur de la propriété selectableList. Doit être appelée avant
	 * setSelectedList.
	 * 
	 * @param newSelectableList List
	 ...
	 * 
	 * @param newSelectableTable SpiTable
	...
	/**
	 * Définit la valeur de la propriété selectionList. Doit être appelée après
	 * setSelectableList.
	 * 
	...
	public void setSelectionList(final List<TypeValue> newSelectionList) {
		if (newSelectionList == null) {
			...
			// enlève tous les objets contenus dans newSelectionList de selectableList s'il
			// y en a
	 ...
	 * 
	 * @param newSelectionTable SpiTable
	...
	private Transferable createListMovementerTransferable(final SpiTable<TypeValue> table) {
		...
		return new Transferable() {
			...
			public DataFlavor[] getTransferDataFlavors() {
				return new DataFlavor[] { LIST_MOVEMENTER_FLAVOR };
			}
