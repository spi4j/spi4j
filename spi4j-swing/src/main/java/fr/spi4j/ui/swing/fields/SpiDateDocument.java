...
public class SpiDateDocument extends SpiTextDocument {
	...
	// Les instances de formats suivants sont partagées pour tous les champs de
	// date.
	// Note : Ce n'est pas gênant même pour un DateFormat car les traitements Swing
	// se font toujours sur un seul thread.
	...
	public static SimpleDateFormat getAlternateDateFormat() {
		if (alternateDateFormat == null) {
			// pour construire ce dateFormat on enlève les '/' et '.' du pattern de
			// dateFormat
			...
			final SimpleDateFormat myAlternateDateFormat2 = new SimpleDateFormat(
					myAlternateDateFormat.toPattern().replaceAll("y", ""));
			final SimpleDateFormat myAlternateDateFormat3 = new SimpleDateFormat(
					myAlternateDateFormat2.toPattern().replaceAll("M", ""));
	 ...
	 * <p>
	 * Ce dateFormat est le format court par défaut de java en fonction de la Locale
	 * par défaut (lenient est false).
	 * 
	 * @return SimpleDateFormat
	 * @see #setDateFormat
	 */
	public static SimpleDateFormat getDateFormat() {
		if (dateFormat == null) {
			// ce dateFormat est bien une instance de SimpleDateFormat selon le source
			// DateFormat.get
	...
	public static SimpleDateFormat getDisplayDateFormat() {
		if (displayDateFormat == null) {
			...
			// mais en saisie on peut toujours n'en mettre que deux qui seront alors
			// interprétés (avec le siècle)
	...
	public void insertString(final int offset, final String string, final AttributeSet attributeSet)
		...
		// pour une date on n'accepte que les chiffres et '/' (et/ou '.', ' ', ':' selon
		// pattern)
		char c;
	 ...
	 * 
	 * @param newAlternateDateFormat SimpleDateFormat
	...
	/**
	 * Définit la valeur de la propriété dateFormat (et réinitialise
	 * alternateDateFormat et displayDateFormat).
	 * <p>
	 * Il est possible notamment de fournir un format acceptant plusieurs syntaxes
	 * ou lançant une RuntimeException "Date invalide" pour l'afficher en rouge si
	 * la date n'est pas entre SpiDateField.getMinimumValue() et
	 * SpiDateField.getMaximumValue().
	 * 
	 ...
	 * 
	 * @param newDisplayDateFormat SimpleDateFormat
