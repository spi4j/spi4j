...
public class SpiBasicTable extends JTable {
	 ...
	 * Constructeur.
	 * 
	 * @param dataModel Modèle pour les données (par exemple, SpiTableModel)
	 */
	...
	/**
	 * Adapte les largeurs des colonnes selon les données de cette table.
	 * <p>
	 * Pour chaque colonne la taille préférée est déterminée selon la valeur (et le
	 * renderer) du header et selon la valeur (et le renderer) de chaque cellule.
	 */
	...
	protected void configureEnclosingScrollPane() {
		// Si cette table est la viewportView d'un JScrollPane (la situation
		// habituelle),
	...
	/**
	 * Retourne la map par défaut des renderers de cellules avec pour clé le type
	 * des valeurs.
	 * 
	 * @return Map
	 */
	...
	/**
	 * Retourne la map par défaut des editors de cellules avec pour clé le type des
	 * valeurs.
	 * 
	 * @return Map
	 */
	private Map<Class<?>, TableCellEditor> getDefaultTableCellEditors() {
		if (DEFAULT_EDITORS.isEmpty()) {
			...
			DEFAULT_EDITORS.put(Date.class,
					new SpiDateActionTableCellEditor(new SpiDefaultCellEditor(new SpiDateField())));
	...
	public TableCellRenderer getDefaultRenderer(final Class<?> columnClass) {
		...
		// (car par ex., le résultat de Collections.unmodifiableList est une interface
		// de List
	...
	protected void keyPressed(final KeyEvent event) {
		...
		// else if (modifiers == 0)
		// {
		// final int selectedColumn = getSelectedColumn() != -1 ? getSelectedColumn() :
		// 0;
