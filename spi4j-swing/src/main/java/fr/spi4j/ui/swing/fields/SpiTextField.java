...
/**
 * Classe abstraite servant de base aux champs de saisie. Cette classe hérite de JTextField. <br/>
 * Pour désactiver globalement, le feedback des champs obligatoires ajouter la propriété système -Dfr.spi4j.ui.swing.fields.SpiTextField.mandatoryFieldsFeedbackDisabled=true <br/>
 * Pour désactiver globalement, la sélection du texte lors du gain de focus, ajouter la propriété système -Dfr.spi4j.ui.swing.fields.SpiTextField.textSelectionOnFocusGained=false <br/>
 * @param <TypeValue>
 *           Type de la valeur saisie
 * @author MINARM
 */
public abstract class SpiTextField<TypeValue> extends JTextField implements HasValue_Itf<TypeValue>, HasMandatory_Itf
{
   static final boolean TEXT_SELECTION_ON_FOCUS_GAINED = Boolean.parseBoolean(System.getProperty(
            SpiTextField.class.getName() + ".textSelectionOnFocusGained", "true"));

   private static final boolean MANDATORY_FIELDS_FEEDBACK_DISABLED = Boolean.parseBoolean(System
            .getProperty(SpiTextField.class.getName() + ".mandatoryFieldsFeedbackDisabled"));
   ...
   private static class FocusHandler implements FocusListener
   {
      ...
      public void focusLost (final FocusEvent focusEvent)
      {
         ...
         if (source instanceof SpiTextField)
         {
            ...
            // v_field.setValue réinitialise v_field.invalidNull, donc il doit être avant "v_field.invalidNull = invalidNull"
   ...
   /**
    * Crée le modèle de document par défaut. <br/>
    * Pour cette classe renvoie une instance de SpiTextDocument, elle sera surchargée dans les classes filles. <br/>
    ...
    * Cette méthode est surchargée dans SpiDateField pour la date du jour, dans SpiHourField pour l'heure courante et dans SpiIntegerField pour +1/-1.
    * @param event
    *           KeyEvent
    */
   ...
   {
      // note : le document n'est pas encore défini lors de l'appel à setEditable dans le constructeur de JTextComponent
      if (getDocument() != null)
      {
         getTextDocument().setEditable(newEditable);
      }
      super.setEditable(newEditable);
      // appel de updateBackground car sans cela la couleur de fond du JTextField non éditable reste blanche et non grisée si le look and feel est Nimbus (à base de Synth)
   ...
   /**
    * Définit la valeur de la propriété text. <br/>
    * Cette méthode est utilisée en interne par les classes filles, elle n'a pas de raison d'être utilisée autrement.
    * @param newText
    *           String
    */
   @Override
   ...
   public void paint (final Graphics g)
   {
      ...
      if (invalidNull)
      {
         ...
         // si le champ est éditable, avec un mapping mandatory et que l'utilisateur est passé sur le champ
         // sans saisir une valeur, on lui donne un feedback pour lui rappeler de saisir ce champ obligatoire
         final Graphics g2 = g.create();
         g2.setColor(INVALID_COLOR);
         final Insets insets = getBorder().getBorderInsets(this);
         g2.drawRect(insets.left - 3, insets.top - 3, getWidth() - insets.right - insets.left + 5, getHeight()
                  - insets.bottom - insets.top + 5);
         g2.dispose();
      }
   }
