...
/**
 * Classe d'impression et d'export. Cette classe s'utilise avec la méthode
 * statique printOrExport.
 * 
 * @author MINARM
 */
public abstract class SpiPrinter {
	...
	protected String buildTitle(final Component component) {
		...
		// sur l'application pilote, ce calcule du titre selon JDialog, JFrame et
		// JTabbedPane n'est pas fiable
	...
	/**
	 * Renvoie le texte de la cellule de cette JTable à la ligne et la colonne
	 * spécifiée.
	 * 
	 * @return String
	 * @param table  SpiBasicTable
	...
	protected String getTextAt(final SpiBasicTable table, final int row, final int column) {
		...
		// suppression de tags éventuellements présents (par ex. avec un
	...
	private String replaceHtml(final String text) {
		if (text != null && text.startsWith("<html>")) {
			...
			result = result.replaceFirst("</html>", ""); // mettre </html> à la fin n'est pas nécessaire, mais au cas où
															// on l'enlève
	...
	/**
	 * Renvoie la valeur de la cellule de cette JTable à la ligne et la colonne
	 * spécifiée.
	 * 
	 * @return String
	 * @param table  SpiBasicTable
	...
	/**
	 * Renvoie la boîte de dialogue swing de choix du fichier d'export. (Initialisée
	 * pour s'ouvrir sur le répertoire courant user.dir).
	 * 
	 * @return JFileChooser
	 */
	...
	protected File chooseFile(final JTable table, final String extension) throws IOException {
		...
		if (title != null) {
			// on remplace par des espaces les caractères interdits dans les noms de
			// fichiers : \ / : * ? " < > |
	...
	/**
	 * Retourne le fichier de sortie, par défaut on ouvre la boîte de dialogue de
	 * choix du fichier.
	 * 
	 * @return File
	 * @param table JTable
	...
	/**
	 * Méthode abstraite : les instances doivent renvoyer l'extension du fichier
	 * exporté.
	 * 
	 * @return String
	 */
	...
	/**
	 * Méthode abstraite : les instances doivent renvoyer l'icône représentant le
	 * type.
	 * 
	 * @return Icon
	 */
	...
	/**
	 * Affiche le document. Les implémentations peuvent surcharger cette méthode.
	 * 
	...
	protected void showDocument(final File targetFile) throws IOException {
		...
		// on pourrait imprimer le fichier directement (par exemple CSV avec Excel) en
		// supposant que Desktop.getDesktop().isDesktopSupported()
