 ...
 * shadow border to all popups except ComboBox popups.
 * 
...
public final class ShadowPopupFactory extends PopupFactory {
	...
	/**
	 * In the case of heavy weight popups, snapshots of the screen background will
	 * be stored as client properties of the popup contents' parent.
	 * <p>
	 * These snapshots will be used by the popup border to simulate the drop shadow
	 ...
	 * Constructeur.
	 * 
	 * @param p_storedFactory PopupFactory
	 */
	private ShadowPopupFactory(final PopupFactory p_storedFactory) {
		super();
		this.storedFactory = p_storedFactory;
	}
	 ...
	 * platforms. Also stores the previously set factory, so that it can be restored
	 * in <code>#uninstall</code>.
	 * <p>
	 * In some Mac Java environments the popup factory throws a NullPointerException
	 * when we call <code>#getPopup</code>.
	 * <p>
	 * The Mac case shows that we may have problems replacing non PopupFactory
	 * instances. Therefore we should consider replacing only instances of
	 * PopupFactory.
	 * 
	 * @see #uninstall()
	 */
	...
	/**
	 * Uninstalls the ShadowPopupFactory and restores the original popup factory as
	 * the new shared popup factory.
	 * 
	 * @see #install()
	 */
	...
	/**
	 * Creates a <code>Popup</code> for the Component <code>owner</code> containing
	 * the Component <code>contents</code>. In addition to the superclass behavior,
	 * we try to return a Popup that has a drop shadow, if popup drop shadows are
	 * active - as returned by <code>Options#isPopupDropShadowActive</code>.
	 * <p>
	 ...
	 * <code>Popup</code> creates to. A null <code>owner</code> implies there is no
	 * valid parent. <code>x</code> and <code>y</code> specify the preferred initial
	 * location to place the <code>Popup</code> at. Based on screen size, or other
	 ...
	 * factory, because the popup type is set in this instance, not in the stored
	 * one.
	 * 
	...
	public static final class ShadowPopup extends Popup {
		...
		/**
		 * The real popup. The #show() and #hide() methods will delegate all calls to
		 * these popup.
		 */
		private Popup popup;
		...
		/**
		 * Returns a previously used <code>ShadowPopup</code>, or a new one if none of
		 * the popups have been recycled.
		 * 
		 * @param owner    component mouse coordinates are relative to, may be null
		 ...
		 * @param delegate the popup to wrap
		 * @return Popup
		 */
		static Popup getInstance(final Component owner, final Component contents, final int x, final int y,
				final Popup delegate) {
			ShadowPopup result;
		 ...
		 * 
		 * @param popup ShadowPopup
		 */
		private static void recycle(final ShadowPopup popup) {
			synchronized (ShadowPopup.class) {
		...
		/**
		 * Hides and disposes of the <code>Popup</code>. Once a <code>Popup</code> has
		 * been disposed you should no longer invoke methods on it. A
		 * <code>dispose</code>d <code>Popup</code> may be reclaimed and later used
		 * based on the <code>PopupFactory</code>. As such, if you invoke methods on a
		 * <code>disposed</code> <code>Popup</code>, indeterminate behavior will result.
		 * <p>
		 * In addition to the superclass behavior, we reset the stored horizontal and
		 * vertical drop shadows - if any.
		 */
		@Override
		public void hide() {
			if (contents == null) {
		 ...
		 * container, we try to snapshot the background. If the <code>Popup</code> is
		 * currently visible, it remains visible.
		 */
		@Override
		public void show() {
			if (heavyWeightContainer != null) {
		 ...
		 * 
		 * @param p_owner    component mouse coordinates are relative to, may be null
		 * @param p_contents the contents of the popup
		 * @param p_x        the desired x location of the popup
		 * @param p_y        the desired y location of the popup
		 * @param p_popup    the popup to wrap
		 */
		private void reset(final Component p_owner, final Component p_contents, final int p_x, final int p_y,
				final Popup p_popup) {
			this.owner = p_owner;
			...
			this.x = p_x;
			this.y = p_y;
			if (p_owner instanceof JComboBox) {
				return;
			}
			...
			final Dimension contentsPrefSize = p_contents.getPreferredSize();
			if (contentsPrefSize.width <= 0 || contentsPrefSize.height <= 0) {
				return;
			}
			for (Container p = p_contents.getParent(); p != null; p = p.getParent()) {
				if (p instanceof JWindow || p instanceof Panel) {
					// Workaround for the gray rect problem.
					p.setBackground(p_contents.getBackground());
					heavyWeightContainer = p;
					break;
				}
			}
			final JComponent parent = (JComponent) p_contents.getParent();
		 ...
		 * <p>
		 * The snapshots are stored as client properties of the contents' parent. The
		 * next time the border is drawn, this background will be used.
		 * <p>
		 * Uses a robot on the default screen device to capture the screen region under
		 * the drop shadow.
		 * <p>
		 * Does <em>not</em> use the window's device, because that may be an outdated
		 * device (due to popup reuse) and the robot's origin seems to be adjusted with
		 ...
		 * 
		 * @param shadowBg    BufferedImage
		 * @param layeredPane Container
		 */
	 ...
	 * popups.
	 * <p>
	 * Can paint the screen background if used with heavy-weight popup windows.
	 * 
	...
	public static class ShadowPopupBorder extends AbstractBorder {
		 ...
		 */
		private static final Image shadow = new ImageIcon(ShadowPopupBorder.class.getResource("/icons/shadow.png"))
				.getImage();
