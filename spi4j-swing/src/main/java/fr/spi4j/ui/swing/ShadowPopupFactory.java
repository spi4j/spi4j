...
/**
 * The JGoodies Looks implementation of <code>PopupFactory</code>. Adds a drop shadow border to all popups except ComboBox popups.
...
public final class ShadowPopupFactory extends PopupFactory
{
   ...
   /**
    * In the case of heavy weight popups, snapshots of the screen background will be stored as client properties of the popup contents' parent. <br/>
    * These snapshots will be used by the popup border to simulate the drop shadow effect. The two following constants define the names of these client properties.
    */
   ...
   /**
    * The PopupFactory used before this PopupFactory has been installed in <code>#install</code>. Used to restored the original state in <code>#uninstall</code>.
    */
   ...
   /**
    * Installs the ShadowPopupFactory as the shared popup factory on non-Mac platforms. Also stores the previously set factory, so that it can be restored in <code>#uninstall</code>.
    * <p>
    * In some Mac Java environments the popup factory throws a NullPointerException when we call <code>#getPopup</code>.
    * <p>
   ...
   /**
    * Creates a <code>Popup</code> for the Component <code>owner</code> containing the Component <code>contents</code>. In addition to the superclass behavior, we try to return a Popup that has a drop shadow, if popup drop shadows are active - as returned by <code>Options#isPopupDropShadowActive</code>.
    * <p>
    * <code>owner</code> is used to determine which <code>Window</code> the new <code>Popup</code> will parent the <code>Component</code> the <code>Popup</code> creates to. A null <code>owner</code> implies there is no valid parent. <code>x</code> and <code>y</code> specify the preferred initial location to place the <code>Popup</code> at. Based on screen size, or other paramaters, the <code>Popup</code> may not display at <code>x</code> and <code>y</code>.
    * <p>
    * We invoke the super <code>#getPopup</code>, not the one in the stored factory, because the popup type is set in this instance, not in the stored one.
    * @param owner
   ...
   /**
    * Does all the magic for getting popups with drop shadows. It adds the drop shadow border to the Popup, in <code>#show</code> it snapshots the screen background as needed, and in <code>#hide</code> it cleans up all changes made before.
    * @author Andrej Golovnin
    * @version 1.4
    */
   public static final class ShadowPopup extends Popup
   {
      ...
      /**
       * Makes the <code>Popup</code> visible. If the popup has a heavy-weight container, we try to snapshot the background. If the <code>Popup</code> is currently visible, it remains visible.
       */
      @Override
      public void show ()
      {
      ...
      /**
       * Snapshots the background.<br/>
       * The snapshots are stored as client properties of the contents' parent. The next time the border is drawn, this background will be used.
       * <p>
       * Uses a robot on the default screen device to capture the screen region under the drop shadow.<br/>
       * Does <em>not</em> use the window's device, because that may be an outdated device (due to popup reuse) and the robot's origin seems to be adjusted with the default screen device.
       * @see #show()
       */
   ...
   /**
    * A border with a drop shadow intended to be used as the outer border of popups.<br/>
