...
/**
 * Renderer d'entête de colonne avec case à cocher "Sélectionner tout" à utiliser en haut d'une colonne de cases à cocher. <br/>
 * Utilisation : new CheckBoxHeaderRenderer(table, column).registerRenderer(); <br/>
 * Remarque : cela ne désactive pas le tri sur les lignes lors du clique sur l'entête de colonne, si jamais le tableau est triable.
 * @author MINARM
 */
public class CheckBoxHeaderRenderer extends JCheckBox implements TableCellRenderer
{
   ...
   /**
    * Si les cases à cocher dans les lignes deviennent toutes cochées ou pas, alors on met à jour le header.
    * @author MINARM
    */
   private class ModelAndSorterListener implements TableModelListener, RowSorterListener
   {
      ...
      private void refreshHeader ()
      {
         ...
         // donc on regarde seulement les lignes affichées dans la vue et pas toutes les lignes du modèle
   ...
   /**
    * Quand on (dé)coche l'entête de la colonne, alors toutes les lignes sont (dé)cochées.
    * @author MINARM
    */
    ...
    *           Le tableau
    * @param pTableColumn
    *           La colonne contenant des booléens (cases à cocher modifiables par exemple)
    */
   ...
   public void registerRenderer ()
   {
      ...
      // Si la table a un RowSorter, alors le clic sur le header de la colonne déclenche l'action toggleSort ce qui n'est pas souhaité,
      // avant que l'évènement mouseClicked n'arrive dans le listener de cette classe.
      // Pour éviter une action de tri en plus de "(Dé)Sélectionner tout", on rend cette colonne non triable.
      if (table.getRowSorter() instanceof DefaultRowSorter<?, ?>)
      {
         // si des colonnes sont ajoutées sur la table après l'appel à setSortable,
         // alors ArrayIndexOutOfBoundsException dans la méthode DefaultRowSorter.isSortable,
   ...
   public static void main (final String[] args)
   {
      ...
      for (int x = 0; x < 5; x++)
      {
         ...
         {Boolean.FALSE, "Row " + (x + 1) + " Col 2", "Row " + (x + 1) + " Col 3" });
      }
