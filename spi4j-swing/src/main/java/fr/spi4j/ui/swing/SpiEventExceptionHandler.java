 ...
 * <p>
 * See javadoc for java.awt.EventDispatchThread#handleException for details.
 * <p>
 * This is sort of a patch to Sun's implementation, which only checks the
 * property once and caches the result ever after.
 * <p>
 * This implementation will always chain to the handler indicated by the current
...
public class SpiEventExceptionHandler {
	...
	/**
	 * Implémentation par défaut de la gestion de l'exception : à surcharger dans
	 * les applications
	 * 
	 ...
	 * <p>
	 * This is kind of a hack, but it's Sun's hack. See the javadoc for
	 * java.awt.EventDispatchThread for details.
	 * <p>
	 * NOTE: we throw an exception immediately, which ensures that our handler is
	 * installed, since otherwise someone might set this property later.
	 * java.awt.EventDispatchThread doesn't actually load the handler specified by
	 * the property until an exception is caught by the event dispatch thread.
	 * <p>
	 * SwingSet2 in 1.4.1 installs its own.
	 * <p>
	 * Note that a new instance is created for each exception thrown.
	 * 
	 * @param p_handlerClass Class
	 * @see "java.awt.EventDispatchThread"
	 */
	public static void install(final Class<?> p_handlerClass) {
		...
		if (isJava17OrMore) {
			// the hack with "sun.awt.exception.handler" throws an exception in Java 1.7, so
			// do nothing here
		...
		if (!SpiEventExceptionHandler.class.isAssignableFrom(p_handlerClass)) {
			throw new IllegalArgumentException(
					"Handler must be derived from " + SpiEventExceptionHandler.class.getName());
		}
	...
	public void handle(final Throwable thrown) {
		...
		if (!handler.equals(getClass().getName())) {
			// Log.debug("A user exception handler (" + handler + ") has been set, invoking
			// it");
			try {
				final ClassLoader cl = Thread.currentThread().getContextClassLoader();
				final Class<?> myClass = Class.forName(handler, true, cl);
				myClass.getMethod("handle", Throwable.class).invoke(myClass.getDeclaredConstructor().newInstance(),
						thrown);
