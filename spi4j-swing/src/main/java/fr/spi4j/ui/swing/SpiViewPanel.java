 ...
 * Panel de vue.
 * 
 * @param <TypePresenter> le type du présenteur de cette vue
 * @author MINARM
 */
public abstract class SpiViewPanel<TypePresenter extends Presenter_Abs<? extends View_Itf, ?>> extends SpiPanel
	 ...
	 * 
	 * @param p_presenter le présenteur de cette vue
	 */
	public SpiViewPanel(final TypePresenter p_presenter) {
		super();
		if (p_presenter == null) {
			// on fait un log à la place d'un throw pour que WindowBuilder puisse afficher
			// le design de l'écran
			LogManager.getLogger(getClass()).error("Le paramètre presenter ne doit pas être null");
			// throw new NullPointerException("Le paramètre presenter ne doit pas être
			// null");
		}
		_presenter = p_presenter;
		_title = getClass().getCanonicalName();

		// initialisation des propriétés name des composants de cette vue en fonction
		// des annotations Graal sur l'interface de la vue
	...
	/**
	 * Méthode à surcharger pour exécuter un traitement lorsque la vue a été
	 * modifiée.
	 * <p>
	 * Attention à ne pas faire de traitement trop lourd dans cette méthode car elle
	 * est appelée à chaque modification d'un champ.
	 * 
	 * @param p_oldValue l'ancienne valeur
	 * @param p_newValue la nouvelle valeur
	 * @param p_event    l'événement source (si existant)
	 */
	 ...
	 * <p>
	 * Cette implémentation vérifie uniquement les champs qui sont marqués avec
	 * setMandatory(true) dans l'interface {@link HasMandatory_Itf}.
	 * 
	 * @return true si tous les champs de type {@link HasMandatory_Itf} sont
	 *         obligatoires (setMandatory(true)) et remplis (non null et toString()
	 *         non vide)
	 */
	public boolean checkMandatoryFields() {
		...
		try {
			...
			// cette exception ne peut pas arriver vu setAccessible(true), mais au cas où on
			// fait un throw
	...
	protected void initNameProperties() {
		try {
			// mantis 1321 pour Marathon pour avoir un nom par défaut même en l'absence
			// d'annotations @Field utilisées ci-dessous
			for (final java.lang.reflect.Field v_field : getClass().getDeclaredFields()) {
				if (!Modifier.isStatic(v_field.getModifiers())) {
					v_field.setAccessible(true);
					final Object v_fieldValue = v_field.get(this);
		...
		// récupère la liste des interfaces (récursivement) de la classe courante, pas
		// plus loin que View_Itf
		...
		for (final Class<?> v_clazz : v_interfaces) {
			...
			// initialisation du name des composants : basé sur @Field des méthodes de
			// l'interface de vue
			for (final Method v_method : v_clazz.getDeclaredMethods()) {
				...
				if (v_method.isAnnotationPresent(Field.class) && v_method.getParameterTypes().length == 0) {
					try {
						...
						// on vérifie que le composant récupéré est bien un Component et on lui met à
						// jour sa propriété name
						...
						// si le getter renvoie une exception, rien d'anormal, on tente les méthodes
						// suivantes
						continue;
					}
				}
	...
	/**
	 * Récupère la liste des interfaces qui contiennent potentiellement les
	 * annotations Field
	 * 
	 * @param p_clazz la classe d'origine
	 * @param p_set   le set des interfaces à retourner (à instancier par
	 *                l'appelant)
	 * @return la liste des interfaces qui contiennent potentiellement les
	 *         annotations Field
	 */
	private Set<Class<?>> getInterfaces(final Class<?> p_clazz, final Set<Class<?>> p_set) {
		// on cherche la superclasse pour aller chercher ses interfaces (pas plus haut
		// que View_Itf)
	 ...
	 * 
	 * @param p_component le composant à écouter
	 */
	...
	/**
	 * @return un DocumentListener pour écouter les modifications sur un champ de
	 *         saisie
	 */
