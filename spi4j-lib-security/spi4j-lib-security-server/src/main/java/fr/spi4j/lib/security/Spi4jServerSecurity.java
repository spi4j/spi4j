...
public final class Spi4jServerSecurity extends Spi4jSecurity_Abs {
	...
	/**
	 * La clé de stockage dans le HttpSession pour les permissions de l'utilisateur
	 * courant.
	 */
	private static final String c_keyPermissions = "permissions";

	/**
	 * La clé de stockage dans le HttpSession pour savoir si l'utilisateur courant
	 * est super admin.
	 */
	private static final String c_keySuperAdmin = "super_admin";

	/**
	 * Retourne l'utilisateur courant dans la session http s'il est connecté.
	 * 
	 * @throws UnauthentifiedUserException si aucun utilisateur n'existe dans la
	 *                                     session http.
	 * @throws SecurityException           si la session http n'existe pas.
	 ...
	 * 
	 * @param p_utilisateur UtilisateurDto
	 */
	@Override
	 ...
	 * 
	 * @param p_initIfUnknown true si il faut initialiser les permissions si elles
	 *                        n'existent pas
	...
	private Set<String> getPermissionsOfCurrentUser(final boolean p_initIfUnknown) {
		...
		if (v_permissions instanceof Set) {
			...
			// les permissions ne sont pas encore initialisées dans la session ou bien
	...
	private void initPermissionsInSession() {
		...
		// On rend le set non modifiable pour être sûr qu'il n'est pas changé
		// insidieusement
