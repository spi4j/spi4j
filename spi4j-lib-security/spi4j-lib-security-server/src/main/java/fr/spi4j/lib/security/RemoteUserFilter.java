...
/**
 * Filtre de servlet positionnant l'identifiant de l'utilisateur dans le contexte ThreadContext de Log4j (%X{user})<br/>
 * et positionnant getRemoteUser() sur la requête http passée à la chaine de servlet<br/>
 * (en particulier, pour avoir cet utilisateur dans la liste des requêtes http en cours et dans la liste des sessions http).
 * @author MINARM
 */
public class RemoteUserFilter implements Filter
{
            ...
            throws IOException, ServletException
   {
      ...
      {
         // il y a déjà un remoteUser dans la requête, ce filtre ne fait rien sur cette requête (hormis positionner le contexte ThreadContext dans Log4j)
      ...
      {
         // cette requête n'est pas encore liée à une session http ou bien l'utilisateur n'est pas encore authentifié avec cette session,
         // ce filtre ne fait rien sur cette requête (il fera probablement quelque chose sur la requête http suivante du même client)
         p_chain.doFilter(p_request, p_response);
         return;
      }
      ...
      // on implémente une requête http identique à celle d'origine hormis que getRemoteUser() retourne l'identifiant de l'utilisateur.
      // Ainsi, le monitoring afficher l'identifiant de l'utilisateur dans les requêtes en cours et dans la liste des sessions http (demande de Didier Féret)
      // et toute autre librairie pourra utiliser la fonction standard JavaEE getRemoteUser() pour connaître l'identifiant de l'utilisateur connecté.
      ...
      // on positionne le contexte ThreadContext dans Log4j : le pattern des messages de log pourra utiliser "%X{user}"
