...
/**
 * Implémentation permettant de centraliser le paramétrage de persistance de
 * l'application.
 *
 * @author MINARM
 */
public final class MyParamPersistence extends ParamPersistence_Abs implements MyParamAppli {
	...
	private static final ResourcePhysical_Abs c_DefaultResourcePhysical = new DefaultResourcePhysical(
			"jdbc:h2:" + ".\\" + c_cheminBaseH2, "ad", "ad", ResourceType_Enum.ressourceH2NonXA);

	/**
	 * Le gestionnaire de connection comprenant des ResourcePhysique et renvoyant le
	 * ResourceManager correspondant au mode de fonctionnement de l'appli.
	 */
	...
	protected void initElemParamPersistence() {
		...
		addElemParamPersistence(MyPersonneEntity_Itf.class,
				new ElemParamPersistence(MyVersionnedPersonneEntity.class, MyPersonneDao.class, _ElemResourceManager));

		// Ajouter le paramétrage pour l'entité "Grade"
		addElemParamPersistence(MyGradeEntity_Itf.class,
				new ElemParamPersistence(MyGradeEntity.class, MyGradeDao.class, _ElemResourceManager));

		// Ajouter le paramétrage pour l'entité "Grade"
		addElemParamPersistence(MyWrongGradeEntity_Itf.class,
				new ElemParamPersistence(MyGradeEntity.class, MyWrongGradeDao.class, _ElemResourceManager));
	}
	...
	/**
	 * Design pattern "Static Holder": Classe pour initialiser au besoin
	 * (c'est-à-dire à la première demande) le userPersistence de l'application sans
	 * nécessiter d'ajouter "synchronized" sur la méthode static
	 * getUserPersistence().
	 * <p>
	 * Ajouter "synchronized" pourrait devenir une contention car la méthode est
	 * static et est appelée très souvent dans l'application.
	 * <p>
	 * Le Static Holder permet d'initialiser l'attribut en étant automatiquement
	 * synchronisé par l'initialisation de la classe dans le ClassLoader.
	 */
