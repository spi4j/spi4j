 ...
 * une liste de Entitys. Seule la méthode statique
 * applyFetchingStrategy(FetchingStrategy, List) doit être utilisée.
 * <p>
 * Les instances de cette classe contiennent un état dépendant de la transaction
 * en cours de leurs fetchingStrategies ; ces instances ne doivent donc pas être
 * ni réutilisées, ni réutilisables.
 *
...
public final class FetchingStrategyFetcher<TypeId> {
	 ...
	 * @param p_entityClass      Classe du Entity
	 */
	// ce constructeur doit rester private pour en maîtriser l'instanciation et la
	// fin de vie
	private FetchingStrategyFetcher(final FetchingStrategy_Abs<TypeId, ? extends Entity_Itf<TypeId>> p_fetchingStrategy,
		...
		try {
			...
			if (_getter_method == null) {
				throw new NoSuchMethodException("Getter de l'attribut " + _attribute.getLogicalColumnName()
						+ " non trouvé pour le Entity" + p_entityClass.getSimpleName());
			}
			if (!List.class.equals(_getter_method.getReturnType())) {
				_getter_method_for_id = EntityAttributeHelper.getInstance().getGetterMethodForAttributeId(p_entityClass,
						_attribute);
				_setter_method = EntityAttributeHelper.getInstance().getSetterMethodForColumn(p_entityClass,
						_attribute);
			} else {
	 ...
	 * @param p_listeEntity      Liste de Entities
	 * @param <TypeId>           Type de l'id dans les Entities
	 * @param <TypeEntity>       Type des Entities qui doit être le même pour la
	 *                           liste et pour la FetchingStrategy
	 */
	 ...
	 * @param p_listeEntity        Liste de Entities
	 * @param <TypeId>             Type de l'id dans les Entites
	 */
	 ...
	 * @param <TypeId>             Type de l'id dans les Entitiess
	 */
	@SuppressWarnings("unchecked")
	private static <TypeId> void applyFetchingStrategy(
			final List<FetchingStrategy_Abs<TypeId, ? extends Entity_Itf<TypeId>>> p_childrenStrategies,
	...
	/**
	 * Application de la fetchingStrategy à tous les Entities de la liste en
	 * paramètre.
	 *
	 * @param p_listeEntity Liste de Entities
	 */
	...
	private void apply(final Entity_Itf<TypeId> p_entity) {
		try {
			if (!List.class.equals(_getter_method.getReturnType())) {
				...
				// et le réseau en réduisant le nombre d'instances de Entites s'il y a plusieurs
				// fois les mêmes
				...
				// fois le getter pour charger le Entity :
				// il faut réutiliser la même instance de Entity sans faire de select pour
				// recharger la même chose

				// Solution : si dans l'application de la fetchingStrategy sur une liste de
				// Entity, les getters concernent des entities de même id,
				// alors on évite d'appeler le getter pour charger le Entity et on récupère
				// l'instance de Entity depuis le cache local
			...
			if (v_result != null) {
				if (v_result instanceof Entity_Itf) {
					...
					// stocke le Entity dans le cache locale à l'instance pour ne pas le recharger
					// si on rencontre le même plus tard dans ce fetcher
	 ...
	 *
	 * @param p_childEntity Entity_Itf
	 */
	...
	/**
	 * Appelle le getter correspondant à cette fetchingStrategy sur le Entity en
	 * paramètre.
	 *
	 * @param p_entity Entity
	...
	/**
	 * Appelle le getter de l'id correspondant à cette fetchingStrategy sur le
	 * Entity en paramètre.
	 *
	 * @param p_entity entity
	...
	/**
	 * Appelle le setter correspondant à cette fetchingStrategy sur le Entity en
	 * paramètre.
	 *
	 * @param p_entity Entity
