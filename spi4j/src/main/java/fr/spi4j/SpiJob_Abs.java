...
/**
 * Classe abstraite de Job (Batch) pour exécuter un traitement de manière
 * répétée et/ou programmée.
 *
 * @author MINARM
 */
public abstract class SpiJob_Abs {
	 ...
	 *
	 * @param p_heure l'heure du traitement (0 --> 23)
	 */
	 ...
	 *
	 * @param p_heure  l'heure du traitement (0 --> 23)
	 * @param p_minute la minute du traitement (0 --> 59)
	 */
	public void scheduleDaily(final int p_heure, final int p_minute) {
		...
		if (v_calendar.getTimeInMillis() < System.currentTimeMillis()) {
			// si l'heure est déjà passé ce jour, alors la prochaine exécution est le
			// lendemain
	 ...
	 *
	 * @param p_secondes l'intervalle de temps en secondes (> 0)
	 */
	...
	/**
	 * Répète le traitement toutes les X heures, à partir de maintenant.
	 *
	 * @param p_secondes l'intervalle de temps en heures (> 0)
	 */
	public void scheduleEveryHours(final int p_hours) {
		if (p_hours <= 0) {
			throw new IllegalArgumentException("Intervalle de temps incorrect (doit être positif): " + p_hours);
		}
		start(new JobConfig(true, p_hours * 3600, new Date()));
	}
	...
	/**
	 * Programme le traitement à une date donnée (si la date est dans le passée, le
	 * traitement est exécuté immédiatement).
	 *
	 * @param p_date la date à laquelle le traitement doit être exécuté
	 */
	...
	/**
	 *
	 * @param day      le jour d'execution.
	 * @param p_heure  l'heure de traitement.
	 * @param p_minute la minute de traitement.
	 */
	public void scheduleWeekly(final int p_day, final int p_heure, final int p_minute) {
		if (p_heure < 0 || p_heure >= 24) {
			throw new IllegalArgumentException("Heure incorrecte (doit être entre 0 et 23 inclus): " + p_heure);
		}
		if (p_minute < 0 || p_minute >= 60) {
		...
		if (p_day < 0 || p_day >= 7) {
			throw new IllegalArgumentException("Jour incorrect (doit être entre 0 et 6 inclus): " + p_day);
		}
		final Calendar v_calendar = Calendar.getInstance();
		v_calendar.set(Calendar.HOUR_OF_DAY, p_heure);
		v_calendar.set(Calendar.MINUTE, p_minute);
		v_calendar.set(Calendar.SECOND, 0);
		v_calendar.set(Calendar.MILLISECOND, 0);
		v_calendar.set(Calendar.DAY_OF_WEEK, p_day);
		start(new JobConfig(true, JobConfig.c_weeklyInterval, v_calendar.getTime()));
	}

	/**
	 * Démarrage du traitement
	 *
	 * @param p_config la configuration du traitement
	 */
	 ...
	 *
	 * @param p_config la config du traitement
	...
	/**
	 * Méthode de gestion des erreurs. Afin de ne pas modifier le fonctionnement
	 * existant (historique), cette methode est laissee en "protected void" mais
	 * elle pourrait passer en "abstract".
	 *
	 * @param p_ex l'erreur lancée par le job
	 */
	 ...
	 *
	 * @throws Throwable exception qui peut être interceptée par la méthode
	 *                   {@link #doOnException(Throwable)}
	 */
	 ...
	 *
	 * @return le UserPersistence de l'application pour gérer une transaction
	 *         globale sur le job
	 */
	protected abstract UserPersistence_Abs getUserPersistence();

	/**
	 * @return true si une transaction doit être gérée par la tache (par défaut), ou
	 *         false sinon
	 */
	...
	private static class JobConfig {
		...
		private static final int c_weeklyInterval = 24 * 60 * 60 * 7;
		 ...
		 * Constructeur.
		 *
		 * @param p_repeat            le job doit-il se répéter ?
		 * @param p_intervalInSeconds combien de secondes séparent 2 jobs (si
		 *                            répétition) ?
		 * @param p_nextRun           à quelle date doit s'exécuter le prochain
		 *                            traitement ?
		 */
		...
		/**
		 * Dans le cas d'un job à répétition, prépare la configuration pour le prochain
		 * traitement
		 *
	...
	/**
	 * Retourne le nom du traitement automatique.
	 *
	 * @return le nom (identifiant) du traitement automatique.
	 */
