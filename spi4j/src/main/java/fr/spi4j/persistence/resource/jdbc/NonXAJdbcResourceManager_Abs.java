...
public abstract class NonXAJdbcResourceManager_Abs extends ResourceManager_Abs implements JdbcResourceManager_Itf {
	 ...
	 *
	 * @param p_currentConnection Connexion courante ou null
	 */
	...
	public void closeCurrentConnection() throws SQLException {
		...
		// s'il n'y a pas de connexion jdbc associée au thread courant dans ce manager
		// de ressource
		// c'est que l'application n'a pas eu besoin de cette ressource et donc il n'y a
		// pas de close à faire
		if (v_connection != null) {
			try {
				v_connection.close();
			} finally {
	...
	/**
	 * Obtention d'une nouvelle connexions jdbc depuis la DataSource (qui va
	 * certainement la récupérer dans son propre pool de connexions)
	 *
	 * @return Connection
	...
	public final Connection getConnection() throws SystemException {
		try {
			// inutile d'utiliser WrapJdbcConnection et un ConnectionEventListener pour
			// purger le _connectionThreadLocal
			...
			// les connexions doivent toujours être en autoCommit false, sinon il n'y a pas
			// de transaction
			...
			// Les informations sont juste affichées dans les logs pour ne pas être remontés
			// au client (faille de sécurité).
			if (_debugInfo != null) {
	...
	protected void initResourcePhysical(final ResourcePhysical_Abs p_ResourcePhysical) throws SystemException {
		// Initialisation des informations de debug (utiles entre autres en cas d'erreur
		// de connexion) : toString de la ressource physique
		...
		try {
			...
			if (JdbcResourceManager_Itf.c_monitoring_enabled) {
				// cela permet d'avoir accès aux statistiques de la base de données à partir du
				// monitoring
				// (en Oracle : connection ouvertes et requêtes en cours, requêtes les plus
				// longues, locks...)
				JdbcWrapper.registerSpringDataSource(p_ResourcePhysical.getUrl(), _dataSource);
				// cela permet d'avoir accès aux statistiques des requêtes sql à partir du
				// monitoring (si WrapJdbcConnection ne le fait pas déjà)
	...
	/**
	 * Appelle commit et close sur toutes les connexions non XA. (s'il y a un
	 * problème, alors appelle rollback et close autant que possible sur toutes les
	 * connexions non XA)
	 *
	 * @throws SystemException e
	 */
	public static void commitAndCloseAllNonXAConnections() throws SystemException {
		try {
			...
			for (final NonXAJdbcResourceManager_Abs v_resourceManager : v_list) {
				// on commit
				// (si exception, le catch Throwable ci-dessous appelera les rollback et surtout
				// tous les close)
			...
			// si il y a une exception ou une error, on essaye de faire des rollback sur ce
			// que l'on peut
			// et dans tous les cas il faut absolument fermer toutes les connexions que l'on
			// peut fermer.
			rollbackAndCloseAllNonXAConnections();

			// puis on dit qu'il y a eu un problème en lançant une exception à l'appelant
			// (si le rollback n'a pas déjà lancé l'exception)
	...
	/**
	 * Appelle rollback et close autant que possible sur toutes les connexions non
	 * XA.
	 *
	 * @throws SystemException e
	 */
	public static void rollbackAndCloseAllNonXAConnections() throws SystemException {
		...
		for (final NonXAJdbcResourceManager_Abs v_resourceManager : v_list) {
			try {
				...
				// si il y a une exception ou une error, on continue à faire rollback et à
				// fermer toutes les connexions que l'on peut fermer.
				// Il faut absolument appeler close() sur toutes les connexions même si une
				// partie se passe mal.
				continue;
			}
		}
		if (v_throwable != null) {
			// puis s'il y a eu une exception ou une error, on dit qu'il y a eu un problème
			// en lançant une exception à l'appelant
	...
	/**
	 * @return Liste des ResourceManager (instances de NonXAResourceManager_Abs et
	 *         si il existe des ResourceManagers XA, alors lance une exception)
	 * @throws SystemException e
	 */
	private static List<NonXAJdbcResourceManager_Abs> getAllNonXAResourceManager() throws SystemException {
		...
		for (final Map<ResourcePhysical_Abs, ResourceManager_Abs> v_mapResourceManager : getAllResourceManager()
				.values()) {
	 ...
	 * createDataSource.
	 *
	 * @param p_resourcePhysical ResourcePhysical
