...
/**
 * Type des resources base de données (Oracle, MySQL ...).<br/>
 * Puisque ces types de ressources ne configurent pas les pools de connexions,<br/>
 * il est indispensable en production d'utiliser une dataSource du serveur d'application dans JNDI et d'utiliser datasourceNonXA ou datasourceXA.
 * @author MINARM
 */
public enum ResourceType_Enum
{
   ...
   // Remarque : pour se connecter à PostgreSQL, utiliser le type de ressource datasourceNonXA et une datasource dans l'annuaire JNDI du serveur
   // ou bien ressourceDbcpNonXA en s'assurant que les dépendances à commons-dbcp et au driver jdbc postgresql sont présentes dans le classpath
   // (car on ne peut paramétrer une datasource du driver Postgresql avec une simple url jdbc)

   /**
    * L'identifiant du gestionnaire de ressource Apache Commons DBCP, sans configuration particulière du pool<br/>
    * (qui peut convenir pour tout type de base de données connectée via une url, un user et un password et dont le driver jdbc est dans le classpath).
    */
