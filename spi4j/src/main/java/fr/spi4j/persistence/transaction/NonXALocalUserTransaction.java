 ...
 * Contrairement à l'implémentation NonXAGlobalUserTransaction, cette implémentation est la plus "conforme" à l'interface UserTransaction.<br/>
 * Elle ne gère pas de transactions locales par thread : il y a une seule transaction pour le thread courant.<br/>
 * Et elle ne gère pas de transactions imbriquées : commit et rollback font réellement le commit ou rollback et le close des connexions.
 * @author MINARM
 */
public class NonXALocalUserTransaction implements UserTransaction
{
   ...
   {
      // begin ne change pas le statut (notamment, on ne repasse pas en status active après un setRollbackOnly())
   }
   ...
   public void commit () throws SystemException
   {
      ...
      // après le commit le status est NO_TRANSACTION, même si le commit fait une erreur
   ...
   {
      // après le rollback le status est NO_TRANSACTION, même si le rollback fait une erreur
      setStatus(Status.STATUS_NO_TRANSACTION);
      NonXAJdbcResourceManager_Abs.rollbackAndCloseAllNonXAConnections();
   }
   ...
   // ces méthodes non publiques sont utilisées dans la gestion interne de NonXAGlobalUserTransaction pour les transactions imbriquées
