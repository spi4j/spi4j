...
/**
 * Instance de javax.transaction.UserTransaction supportant des connexions jdbc multiples au mieux mais sans gestion de XA.<br/>
 * Comme pour l'implémentation XA "UserTransactionImpl", une instance de cette classe est partageable par plusieurs threads et des transactions locales par thread sont gérées dans cette implémentation.<br/>
 * Et cette implémentation gère également des transactions imbriquées (par exemple, begin / begin / commit / rollback en séquence) : la transaction extérieure est seule maître.<br/>
 * Mais contrairement à l'implémentation XA "UserTransactionImpl", cette implémentation ne gère pas son propre pool de resources et de connexions et fait réellement les close() sur les connexions après les commit et rollback.
 * @author MINARM
 */
public class NonXAGlobalUserTransaction implements UserTransaction
{
   ...
   /**
    * Attribut static car les transactions doivent être globales à la JVM pour un thread.<br/>
    * Par exemple, pour 2 instances de UserPersistence contenant 2 instances de ParamPersistence contenant 2 instances de NonXAGlobalUserTransaction,<br/>
    * il ne doit y avoir qu'une et une seule transaction par thread (donc 1 seule instance de NonXALocalUserTransaction par thread).
    */
    ...
    *            Une SystemException est lancée s'il n'y a pas de transaction associée au thread courant (begin n'a pas été appelée)
    */
   ...
   /**
    * Définit la transaction du thread courant, qui peut être null s'il n'y en a plus
    * @param p_currentTransaction
    *           Transaction courante ou null
    */
