...
/**
 * Classe abstraite permettant de centraliser tous les traitements de
 * persistance.
 *
 * @author MINARM
 */
public abstract class UserPersistence_Abs {
	 ...
	 * Constructeur privé.
	 *
	 * @param p_ParamPersistence Le ParamPersistence
	 */
	 ...
	 *
	 * @param <T>     Type générique de l'entity
	 * @param p_alias (In)(*) L'alias de l'entité désirée.
	 ...
	 *
	 * @param <T>     Type générique du DAO
	 * @param p_alias (In)(*) L'alias du DAO désiré.
	 * @return L'instance désirée.
	 */
	@SuppressWarnings("unchecked")
	...
	private ElemParamPersistence getElemParamPersistence(final Class<? extends Entity_Itf<?>> p_alias) {
		// Obtenir l'instance décrivant le triplet (Entity, DAO, Ressource) pour l'alias
		// 'p_alias'
	 ...
	 *
	 * @param <T>     Type du résultat (évite à l'appelant de faire le cast)
	 * @param p_Class (In)(*) La classe à instancier (nécessite un constructeur par
	 *                défaut).
	 * @return L'instance désirée.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Vérifie la cohérence de la base de données par rapport à la version de
	 * l'application Java.
	 */
	...
	/**
	 * Permet d'indiquer aux DaoJdbc que les requêtes select formées doivent
	 * contenir le nom de toutes les colonnes au lieu de simplement *.
	 * <p>
	 * Exemple :
	 * <li>select PERSONNE.ID, PERSONNE.NOM, PERSONNE.PRENOM from PERSONNE</li>
	 * <p>
	 * au lieu de
	 * <li>select PERSONNE.* from PERSONNE</li>
	 */
	protected static void useColumnsInSelect() {
		// Nom de la propriété système indiquant si les selects doivent contenir le nom
		// de toutes les colonnes ou seulement * (par défaut en JDBC)
