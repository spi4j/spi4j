         ...
         implements Dao_Itf<Long, TypeEntity, Columns_Enum>
{
   ...
   /**
    * Propriété indiquant si les selects doivent contenir le nom de toutes les colonnes ou seulement * (par défaut en JDBC)<br />
    * Attention, cette propriété est dupliquée dans DaoJdbc_Abs (là ou elle est écrite)
    */
   private static final boolean c_useColumnsInSelect = Boolean.parseBoolean(System.getProperty(
            "jdbc.useColumnsInSelect", Boolean.toString(false)));
    ...
    * @param p_columnNames
    *           (In) Tableau de l'énumération des colonnes dans cette table
    */
    ...
    * @param p_columnNames
    *           (In) Tableau de l'énumération des colonnes dans cette table
    * @param p_additionalColumnNames
    *           (In) Tableau de l'énumération des colonnes additionnelles dans cette table
    */
   public DaoJdbc_Abs (final String p_tableName, final ColumnsNames_Itf[] p_columnNames,
   ...
   public ResourceManager_Abs getResourceManager ()
   {
      ...
      {
         throw new Spi4jRuntimeException(null, "Aucun gestionnaire de ressource n'a été associé au DAO : "
                  + this.getClass(), "???");
      }
   ...
   /**
    * Construit une instance d'entity correspondant à ce DAO à partir d'une Map des valeurs de l'entity, ayant les noms logiques des colonnes comme clés.
    * @param p_map_valueByColumnName
    *           Map
    * @return Map
    */
      ...
      if (v_entityHasId)
      {
         ...
         else
         {
            throw new Spi4jRuntimeException(
                     "L'entité insérée possède déjà un identifiant.",
   ...
   /**
    * Retourne la même chose que getMapValueByLogicalNameFromEntity, mais sans la valeur de l'id.
    * @param p_Entity
    *           Entité
    * @return Map
   ...
   public void update (final TypeEntity p_Entity) throws Spi4jValidationException
   {
      ...
      if (v_entityVersionned)
      {
         ...
         // récupération de la version avant mise à jour et mise à jour de la nouvelle version
         final Object v_oldLockVersion = JdbcVersionHelper.updateVersionForEntity(p_Entity);
         // construction de la map des paramètres
         v_map_valueByLogicalName = getMapValueByLogicalNameWithoutId(p_Entity);
         // test que la version de l'entité en cours est la bonne avec ajout du paramètre de l'ancienne version
   ...
   /**
    * Renvoie une erreur si aucune ligne n'a été mise à jour (delete / update), entité versionnée ou non
    * @param p_Entity
    *           l'entité
    * @param p_entityVersionned
    *           true si l'entité est versionnée, false sinon
    */
   ...
   /**
    * Retourne une liste d'entités à partir d'un Cursor.<br />
    * Ce Cursor peut avoir été créé par la méthode {@link #findByCriteria(String, Map)} ou bien par la méthode {@link #executeQuery(String, Map, String...)}<br />
    * Exemple : <br />
    * <code>
   ...
   /**
    * Construit la map des valeurs pour la ligne lue dans le Curseur. <br />
    * La map ne contiendra que les valeurs connues pour l'entité associée à ce DAO.
    * @param p_cursor
    *           le curseur lu
    * @param p_map_valueByLogicalName
    *           la map des valeurs à mettre à jour
    */
   ...
   {
      // optimisation : clear pas nécessaire puisqu'on redéfinit toutes les valeurs à chaque ligne du curseur
      // et il est plus optimisé de ne pas faire clear() pour éviter de réinstancer à chaque ligne autant de Map.Entry que de colonnes
   ...
   private String buildSqlProjection ()
   {
      ...
      if (c_useColumnsInSelect)
      {
         ...
         for (final ColumnsNames_Itf v_columnName : _columnNames)
         {
            v_stringBuilder.append(_tableName).append('.').append(v_columnName.getPhysicalColumnName()).append(", ");
         }
   ...
   /**
    * Retourne la valeur d'une colonne dans le curseur en paramètre à la ligne courante de ce curseur.
    * @param p_cursor
    *           Curseur
    * @param p_column
    *           Colonne (ou alias) dans la requête select
    * @return Valeur
    */
   ...
   /**
    * Retourne des messages en français pour les utilisateurs et les développeurs de certaines erreurs courantes dans une base de données Oracle.
    * @param p_exception
    *           Exception SQL
    * @param p_defaultMessage
    *           Message par défaut
            ...
            final int p_nbLignesMax, final int p_nbLignesStart, final String... p_commentaires)
   {
      ...
      if (v_databaseType == DatabaseType.ORACLE)
      {
         // mantis spi4j #1889 : il a été constaté un bug Oracle (sur un cas contenant un rownum pour p_nbLignesMax et quand certaines colonnes sont sélectionnées)
         // certaines lignes ne sont parfois pas toutes retournées (base 10.2.0.3.0 et driver jdbc 11.2.0.3.0, non reproduit avec Toad pour la même requête).
         // Il a été constaté que dans ce cas, l'ajout d'un espace à la fin (après le rownum) permet de contourner le problème.
      ...
      try
      {
         ...
         {
            // si la requête n'a pas pu être optimisée, alors on il faut que le curseur se positionne au bon endroit (temps important)
   ...
   /**
    * Permet d'obtenir la connexion JDBC du thread courant (non null). La ressource est automatiquement associée à la transaction du thread local. <br/>
    * Cette méthode lance une exception avec un message explicatif pour l'utilisateur si la connexion jdbc n'a pas pu être établi.
   ...
   private Connection getCurrentConnection ()
   {
      ...
      {
         // pas Spi4jRuntimeException ici pour avoir un message clair pour les utilisateurs dans le cas où la base de données n'est pas accessible
   ...
   /**
    * Crée une instance de JdbcQueryBuilder correspondant à la table et aux colonnes de ce DAO.
