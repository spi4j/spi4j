...
/**
 * Classe abstraite permettant de centraliser toutes les traitements communs aux
 * DAO.
 *
 * @author MINARM
...
abstract public interface Dao_Itf<TypeId, TypeEntity extends Entity_Itf<TypeId>, Columns_Enum extends ColumnsNames_Itf> {
	 ...
	 *
	 * @param p_ResourceManager (In) Le gestionnaire de ressource associé au DAO.
	 ...
	 * Créer un tuple.
	 *
	 * @param p_Entity (In/Out)(*) Le tuple à inséer est obligatoire (sauf la clé
	 ...
	 * @return Le tuple désiré (non null ou exception si donnée non trouvée en base
	 *         de données).
	 */
	...
	/**
	 * Mettre à jour le tuple (et lance une exception si donnée non trouvée en base
	 * de données).
	 *
	 * @param p_Entity (In)(*) Le tuple à mettre à jour est obligatoire.
	 * @throws Spi4jValidationException Si le tuple n'est pas valide
	 */
	void update(TypeEntity p_Entity) throws Spi4jValidationException;

	/**
	 * Supprimer un tuple à partir de sa clé primaire (et lance une exception si
	 * donnée non trouvée en base de données).
	 *
	 ...
	 * nommés et de nombres de lignes pour une pagination.
	 * <h4>Exemple:</h4>
	 *
	 * <pre>
	 * final List&lt;PersonneEntity_Itf&gt; v_personnes = personDao.findByCriteria(&quot;where grade_id &gt;= :grade_id&quot;,
	...
	/**
	 * Obtenir la liste des tuples à partir d'un TableCriteria (le critère sql sera
	 ...
	 * @param p_map_value_by_name (In) Map des valeurs des paramètres selon les noms
	 *                            de paramètres (non obligatoire).
	 * @param p_commentaires      (In) Commentaire(s) décrivant la requête (non
	 *                            obligatoire : la méthode peut être spécifier aucun
	 *                            commentaire)
	...
	/**
	 * Permet d'exécuter une requête de sélection, avec des nombres de lignes pour
	 * une pagination.
	 *
	 * @param p_query             (In)(*) La requête (obligatoire).
	 * @param p_map_value_by_name (In) Map des valeurs des paramètres selon les noms
