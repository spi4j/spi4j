...
/**
 * Intercepteur pour EJB 3 qui met en cache par JVM les résultats des appels de
 * findById et de findAll.
 * <p>
 * (utile par exemple, pour limiter les appels du client sur le serveur sur des
...
public final class ServiceCacheInterceptor implements Serializable {
	 ...
	 *
	 * @param p_context InvocationContext
	 * @return Object
	 * @throws Exception e
	 */
	@AroundInvoke
	public Object intercept(final InvocationContext p_context) throws Exception {
		...
		// on garde un cache de ces ServiceCacheProxy pour éviter de les réinstancier à
		// chaque fois
	...
	/**
	 * Retourne une instance de ServiceCacheProxy pour l'interface de service en
	 * paramètre.
	 *
	 * @param p_interfaceService Class
	...
	private static ServiceCacheProxy<?> getServiceCacheProxyFromCache(final Class<?> p_interfaceService) {
		...
		if (v_serviceCacheProxy == null) {
			// il n'y aura pas besoin du service juste pour lookInCache et putInCache donc
			// on met null pour le service
