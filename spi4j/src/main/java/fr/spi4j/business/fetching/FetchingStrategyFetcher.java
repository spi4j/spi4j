...
/**
 * Classe permettant de fetcher, et donc d'appliquer une FetchingStrategy, sur une liste de DTOs.<br/>
...
public final class FetchingStrategyFetcher<TypeId>
{
            ...
            final Class<? extends Dto_Itf<TypeId>> p_dtoClass)
   {
      ...
      try
      {
         ...
         {
            throw new NoSuchMethodException("Getter de l'attribut " + _attribute.getName() + " non trouvé pour le DTO "
                     + p_dtoClass.getSimpleName());
         }
    ...
    *           Liste de DTOs
    * @param <TypeId>
    *           Type de l'id dans les DTOs
    * @param <TypeDto>
    *           Type des DTOs qui doit être le même pour la liste et pour la FetchingStrategy
    */
            ...
            final List<? extends Dto_Itf<TypeId>> p_listeDto)
   {
      if (!p_childrenStrategies.isEmpty() && !p_listeDto.isEmpty())
      {
         for (final FetchingStrategy_Abs<TypeId, ? extends Dto_Itf<TypeId>> v_fetchingStrategy : p_childrenStrategies)
         {
            ...
            {
               final Class<? extends Dto_Itf<TypeId>> v_dtoClass = (Class<? extends Dto_Itf<TypeId>>) p_listeDto.get(0)
                        .getClass();
            ...
            final Dto_Itf<TypeId> p_dto)
   {
      if (!p_childrenStrategies.isEmpty())
      {
         for (final FetchingStrategy_Abs<TypeId, ? extends Dto_Itf<TypeId>> v_fetchingStrategy : p_childrenStrategies)
         {
            if (isActive(v_fetchingStrategy))
            {
               final Class<? extends Dto_Itf<TypeId>> v_dtoClass = (Class<? extends Dto_Itf<TypeId>>) p_dto.getClass();
               final FetchingStrategyFetcher<TypeId> v_fetchingStrategyFetcher = new FetchingStrategyFetcher<>(
                        v_fetchingStrategy, v_dtoClass);
   ...
   private void apply (final Dto_Itf<TypeId> p_dto)
   {
      try
      {
         if (!List.class.equals(_getter_method.getReturnType()))
         {
            // Objectif : économiser le nombre de selects unitaires et économiser la mémoire et le réseau en réduisant le nombre d'instances de DTOs s'il y a plusieurs fois les mêmes

            // Exemple : pour une liste d'opérations d'infrastructure d'un projet donné (ou de quelques projets),
            // alors le fetch des projets de chaque opération doit éviter d'appeler chaque fois le getter pour charger le DTO :
            // il faut réutiliser la même instance de DTO sans faire de select pour recharger la même chose

            // Solution : si dans l'application de la fetchingStrategy sur une liste de DTO, les getters concernent des DTOs de même id,
            // alors on évite d'appeler le getter pour charger le DTO et on récupère l'instance de DTO depuis le cache local
            ...
            if (v_result != null)
            {
               ...
               // (pas besoin d'appliquer les fetchingStrategy filles puisque cela a déjà été fait pour ce DTO fils)
         ...
         if (v_result != null)
         {
            if (v_result instanceof Dto_Itf)
            {
               ...
               // stocke le DTO dans le cache locale à l'instance pour ne pas le recharger si on rencontre le même plus tard dans ce fetcher
   ...
   /**
    * Appelle le getter correspondant à cette fetchingStrategy sur le DTO en paramètre.
    * @param p_dto
    *           DTO
   ...
   /**
    * Appelle le getter de l'id correspondant à cette fetchingStrategy sur le DTO en paramètre.
    * @param p_dto
    *           DTO
   ...
   /**
    * Appelle le setter correspondant à cette fetchingStrategy sur le DTO en paramètre.
    * @param p_dto
    *           DTO
    * @param p_value
