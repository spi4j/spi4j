...
/**
 * Proxy des interfaces de services qui met en cache par JVM les résultats des
 * appels de findById et de findAll. (utile par exemple, pour limiter les appels
 * du client sur le serveur sur des données du référentiel)
 *
...
public final class ServiceCacheProxy<TypeService> implements InvocationHandler {
	...
	// et il est possible d'utiliser la librairie EhCache avec configuration
	// possible dans un fichier ehcache.xml dont la taille :
	 ...
	 * Constructeur privé.
	 *
	 * @param p_interfaceService La classe de service qui aura du cache
	 ...
	 *
	 * @param p_cacheFactory la factory de caches
	 */
	public static void initCacheFactory(final CacheFactory_Itf p_cacheFactory) {
		if (p_cacheFactory == null) {
			throw new IllegalArgumentException(
					"Le paramètre 'cacheFactory' est obligatoire, il ne peut pas être 'null'\n");
		}
		cacheFactory = p_cacheFactory;
	}

	/**
	 * Crée une instance d'un cache.
	 * <p>
	 * Le type de cache peut être configuré en injectant une instance de
	 * CacheFactory_Itf avec initCacheFactory.
	 *
	...
	public static <TypeService> TypeService createProxy(final Class<TypeService> p_interfaceService,
		...
		final ServiceCacheProxy<TypeService> v_serviceCacheProxy = new ServiceCacheProxy<>(p_interfaceService,
				p_service);
		return (TypeService) Proxy.newProxyInstance(p_service.getClass().getClassLoader(),
				p_service.getClass().getInterfaces(), v_serviceCacheProxy);
	}
	 ...
	 *
	 * @param p_method la méthode appelée
	 * @param p_args   les arguments de la méthode
	 * @return le résultat de la méthode
	 * @throws Throwable si erreur d'invocation du service
	 */
	 ...
	 * @param p_method La méthode appelée
	 * @param p_args   Les arguments de la méthode
	 * @return L'objet trouvé dans le cache s'il existe, ou null si le résultat de
	 *         cette méthode n'a pas été trouvé dans le cache
	 */
	Object lookInCache(final Method p_method, final Object[] p_args) {
		...
		// alors on regarde dans le cache si on a le résultat (sinon on fera l'appel au
		// service distant)
		// remarque: on pourrait généraliser ce cache à toutes les méthodes avec
		// paramètres,
		// mais les méthodes find avec paramètres ne sont pas forcément utiles comme
		// cache en général)
	 ...
	 *
	 * @param p_id Identifiant de l'objet
	 ...
	 * @param p_method         La méthode appelée
	 * @param p_args           Les arguments de la méthode
	 * @param p_resultOfInvoke Le résultat de la méthode
	 */
	 ...
	 *
	 * @param p_identifiable L'objet à rajouter en cache
	 */
	 ...
	 *
	 * @param p_method         Méthode
	 * @param p_resultOfInvoke Résultat de la méthode
	 */
	@SuppressWarnings("unchecked")
	private void putNoArgsResultInCache(final Method p_method, final Object p_resultOfInvoke) {
		...
		if ("findAll".equals(p_method.getName()) && p_resultOfInvoke instanceof Collection) {
			// si on a la liste des objets identifiables après findAll, autant mettre chacun
			// des obetss dans le cache pour findById
