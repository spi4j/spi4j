...
/**
 * Implémentation de javax.servlet.Filter qui place toutes les requêtes http passant par le filtre dans une transaction. <br>
 * Le but est que même si les services sont chacun transactionnels, alors les appels de services successifs sont globalement dans une transaction, <br/>
 * par exemple pour des appels de services successifs depuis une servlet d'un client léger web (jsf, jsp, etc).<br/>
 * Lors de l'initialisation de la webapp, il faut appeler la méthode "TransactionFilter.initUserPersistence(XxParamPersistence.getUserPersistence())" <br/>
 * pour que ce filtre connaisse le "userPersistence" permettant de gérer la transaction.
 * @author MINARM
 */
