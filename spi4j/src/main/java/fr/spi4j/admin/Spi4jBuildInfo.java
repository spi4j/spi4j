...
/**
 * Récupèration ou affichage des informations sur le build.
 * <p>
 * (clés : build.number, build.id, build.svnRevision, build.svnUrl,
 * build.projectVersion, etc telles que définies dans le pom.xml)
 *
 * @author MINARM
 */
public final class Spi4jBuildInfo extends BuildInfo_Abs {
	...
	private Map<String, String> loadInfos() {
		...
		try {
			...
			// n'est pas censé arrivé puisque le stream et donc le fichier existe à cet
			// endroit
		...
		for (final Entry<Object, Object> v_entry : v_properties.entrySet()) {
			...
			// on ne garde que les propriétés correspondant au pattern souhaité et pas les
			// autres propriétés
