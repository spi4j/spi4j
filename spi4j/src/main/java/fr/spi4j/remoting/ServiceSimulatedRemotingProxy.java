...
/**
 * Proxy des interfaces de services pour faire des simulations d'appels distants
 * entre une application cliente et un serveur (via
 * sérialisation/désérialisation du premier appel pour éviter le passage par
 * référence).
 *
...
public final class ServiceSimulatedRemotingProxy<TypeService> implements InvocationHandler {
	 ...
	 * Constructeur privé.
	 *
	 * @param p_interface La classe du composant dont le nom sera mis dans les logs
	 * @param p_delegate  Composant
	 */
	...
	/**
	 * Factory de création du proxy de simulation du remoting pour le service (via
	 * sérialisation/désérialisation du premier appel pour éviter le passage par
	 * référence).
	 *
	 * @param <TypeService> Le type du service
	 * @param p_interface   La classe du composant dont le nom sera mis dans les
	 *                      logs
	 * @param p_delegate    Le composant
	...
	public static <TypeService> TypeService createProxy(final Class<TypeService> p_interface,
		...
		return (TypeService) Proxy.newProxyInstance(p_delegate.getClass().getClassLoader(),
				p_delegate.getClass().getInterfaces(), v_serviceSimulatedRemotingProxy);
	}
	...
	public Object invoke(final Object p_proxy, final Method p_method, final Object[] p_args) throws Throwable {
		if (!isInService()) {
			...
			// si on n'est pas déjà entré dans un service pour ce thread, alors c'est le
			// premier appel de service (non imbriqué),
			// donc on sérialise en mémoire et on désérialise les paramètres de la méthode
			// pour éviter le passage par référence
			try {
			...
			// on est déjà entré dans un service, donc il s'agit d'un service imbriqué et on
			// ne fait rien de spécial
			// (pour un service imbriqué, il ne faut pas empêcher d'utiliser le passage par
			// référence)
	...
	private void logMessage(final Method p_method, final long p_start) {
		if (c_logger.isInfoEnabled()) {
			...
			// pour pouvoir activer de temps en temps en développement des traces disant
			// depuis quels endroits du code ont été fait les appels au remoting
	...
	private static Object[] deepClone(final Object[] p_args) {
		...
		try {
			...
			final ObjectInputStream v_input = new ObjectInputStream(
					new ByteArrayInputStream(v_byteArray.toByteArray()));
			try {
