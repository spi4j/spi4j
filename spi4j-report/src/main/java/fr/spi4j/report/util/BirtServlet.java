...
/**
 * Servlet servant d'exemple pour utilisation de BIRT dans une webapp.
 * 
 * @author MINARM
 */
public class BirtServlet extends HttpServlet {
	...
	protected void doGet(final HttpServletRequest p_req, final HttpServletResponse p_resp)
		...
		// ceci n'est qu'un exemple, et dans la vraie vie il faudrait avoir un
		// "contrôleur" qui prépare les données pour ce report en particulier
		...
		// dans le cas simple indiqué en commentaire ci-dessus, il est équivalent et
		// plus efficace de faire directement comme ceci :
		v_pageBirt.writeReport(v_outputStream);
	}

	@Override
	public void init(final ServletConfig p_config) throws ServletException {
		// On n'initialise pas la plateforme Birt lors du démarrage de la webapp, car
		// c'est trop long en tests : elle sera initialisée au besoin à la première
		// utilisation
