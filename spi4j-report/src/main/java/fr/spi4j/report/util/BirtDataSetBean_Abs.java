...
/**
 * Conteneur de beans abstrait (à hériter pour définir une classe correspondant
 * aux données souhaitées et passées dans le constructeur).
 * <p>
 * En alternative à l'utilisation de cette classe, il suffirait d'hériter de
 * ScriptedDataSetEventAdapter et d'appeler dans la méthode fetch :
 * "p_row.setColumnValue(v_attributeName, v_value)" pour chaque attribut.
 * 
 * @author MINARM
 */
public abstract class BirtDataSetBean_Abs extends DataSetEventAdapter_Abs {
	 ...
	 * 
	 * @param p_dataSet       Le data set.
	 * @param p_reportContext Report context.
	 */
	@Override
	 ...
	 * Methode de la classe ScriptedDataSetEventAdapter
	 * 
	 * @param p_dataSet Le dataSet.
	 * @param p_row     ?
	 * @return true ou false selon la taille du tableau (mais ce n'est pas très
	 *         clair).
	 */
	@Override
	 ...
	 * 
	 * @param p_listBeans Liste de beans (par exemple, une liste de DTOs).
	 * @return La Map avec les getters.
	 */
	private Map<DataContainer, Method> buildGetters(final List<?> p_listBeans) {
		final HashMap<DataContainer, Method> v_gettersByDataContainer = new HashMap<>(_listDataContainer.size());
		...
		if (!p_listBeans.isEmpty()) {
			...
			try {
				for (final DataContainer v_dataContainer : _listDataContainer) {
					...
					// récupère le getter (le getter doit avoir pour nom "get" + le columnName du
					// dataContainer)
				...
				throw new ReportException("Problème pour constituer les getters avec v_columnName=" + v_columnName,
						v_e);
			}
		}
