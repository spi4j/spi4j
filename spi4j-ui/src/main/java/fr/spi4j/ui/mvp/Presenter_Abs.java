...
/**
 * L'ancêtre des présenteurs de vue.<br/>
 * Chaque instance de présenteur est lié à une et une seule instance de vue et à une et une seule instance Dto/Entity.
 * @param <TypeView>
 *           le type de vue présentée
 * @param <T>
 *           le type de (DTO/Entity) affiché, ou Object si aucun (DTO/Entity)
 * @author MINARM
 */
public abstract class Presenter_Abs<TypeView extends View_Itf, T>
{
   ...
   /**
    * Initialisation du présenteur. Méthode appelée entre beforeCreate et afterCreate du {@link PresenterListener_Itf}.
    */
   ...
   /**
    * Méthode d'initialisation de la vue (après instanciation). Cette méthode sert à remplir les champs de la vue, comme par exemple les valeurs possibles dans une liste. Elle sert également à instancier les vues enfant.
    */
    ...
    * @param p_view
    *           la vue associée à ce présenteur
    */
    ...
    * @return true (par défaut) si lorsque ce présenteur se ferme, il doit demander la fermeture de ses enfants automatiquement, false sinon
    */
   ...
   /**
    * @return le DTO/entity associé à ce présenteur (s'il a été affecté précédement)
    */
   public T getObjet ()
   {
      return _objet;
   ...
   /**
    * Génération d'un id pour le présenteur. Implémentation par défaut : génère l'id par défaut. Méthode à surcharger pour les présenteurs pouvant être instanciés plusieurs fois.
    ...
    * @param p_event
    *           l'événement envoyé par le présenteur
    */
   ...
   /**
    * Ecoute un certain type d'événement. Tout événement envoyé de ce type sera intercepté dans la méthode onEvent.
    * @param p_eventType
    ...
    * @param p_eventType
    *           le type d'événément qui ne sera plus écouté par ce présenteur.
    */
   ...
   /**
    * Réception d'un événement. Méthode à surcharger pour intercepter les événements après un listen.
    * @param p_event
